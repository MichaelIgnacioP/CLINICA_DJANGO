...
#para vista signup/login boton del admin
...
#para vista signup/login boton del doctor
...
#para vista signup/login boton del paciente
...
#-----------chequear si es usuario, paciente, doctor o admin
def is_admin(user):
...
#---------DESPUÉS DE INGRESAR LAS CREDENCIALES VERIFICAMOS SI EL NOMBRE DE USUARIO Y LA CONTRASEÑA ES DE ADMIN, DOCTOR O PACIENTE
...
#---------------------------------------------------------------------------------
#------------------------ INICIO DE LAS VISTAS RELACIONADAS CON EL ADMINISTRADOR ------------------------------
#---------------------------------------------------------------------------------
...
# esta vista para la barra lateral, haga clic en la página de administración
@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
...
def admin_aprobar_doctor_vista(request):
    #aquellos cuya aprobación es necesaria
...
#------------------PARA APROBAR PACIENTE POR EL ADMIN----------------------
@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
...
#--------------------- PARA DAR DE ALTA AL PACIENTE POR ADMIN-------------------------
@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
...
def alta_paciente_vista(request,pk):
    paciente=models.Paciente.objects.get(id=pk)
    dias=(date.today()-paciente.fechaAdmision) #2 dias, 0:00:00
    doctorAsignado=models.User.objects.all().filter(id=paciente.doctorAsignadoId)
    d=dias.days # solo cuando la cantidad de dias es dos
    pacienteDict={
        'pacienteId':pk,
        ...
        'dia':d,
        'nombreDoctorAsignado':doctorAsignado[0].first_name,
    }
    if request.method == 'POST':
        tarifaDict ={
        ...
        #para actualizar la base de datos de DetallesAltaPaciente (pDD)
        pDD=models.DetallesAltaPaciente()
        pDD.pacienteId=pk
        pDD.nombrePaciente=paciente.get_name
        pDD.nombreDoctorAsignado=doctorAsignado[0].first_name
...
#--------------para descargar e imprimir la factura del paciente (pdf)
import io
...
def download_pdf_view(request,pk):
    detallesAlta=models.DetallesAltaPaciente.objects.all().filter(patientId=pk).order_by('-id')[:1]
    dict={
...
def admin_agregar_agenda_vista(request):
    ...
    if request.method=='POST':
        ...
        if agendaForm.is_valid():
            ...
            agenda.nombrePaciente=models.User.objects.get(id=request.POST.get('pacienteId')).first_name
            agenda.status=True
            agenda.save()
...
#---------------------------------------------------------------------------------
#------------------------ FIN DE LAS VISTAS RELACIONADAS CON EL ADMINISTRADOR ------------------------------
#---------------------------------------------------------------------------------
...
#---------------------------------------------------------------------------------
#------------------------ VISTAS RELACIONADAS CON EL DOCTOR ------------------------------
#---------------------------------------------------------------------------------
@login_required(login_url='doctorlogin')
@user_passes_test(is_doctor)
def doctor_dashboard_vista(request):
    #TRES DATOS POR DOCTOR REFERENTES AL PACIENTE
    ...
    #INFO DASHBOARD DOCTOR/PACIENTES/AGENDAS
    ...
    'agendas':agendas,
    'doctor':models.Doctor.objects.get(user_id=request.user.id), #para la foto de perfil del doctor en la barra lateral
    }
...
def paciente_agenda_vista(request):
    paciente=models.Paciente.objects.get(user_id=request.user.id) #para la foto de perfil del paciente en la barra lateral
...
def paciente_libro_agenda_vista(request):
    ...
    if request.method=='POST':
        ...
        if agendaForm.is_valid():
            ...
            agenda=agendaForm.save(commit=False)
            agenda.doctorId=request.POST.get('doctorId')
            agenda.pacienteId=request.user.id #----el usuario puede elegir cualquier paciente, pero solo se almacenará su información
            agenda.nombreDoctor=models.User.objects.get(id=request.POST.get('doctorId')).first_name
            agenda.nombrePaciente=request.user.first_name #----el usuario puede elegir cualquier paciente, pero solo se almacenará su información
            agenda.status=False
            agenda.save()
...
def paciente_alta_vista(request):
    paciente=models.Paciente.objects.get(user_id=request.user.id) #para la foto de perfil del paciente en la barra lateral
    detallesAlta=models.DetallesAltaPaciente.objects.all().filter(pacienteId=paciente.id).order_by('-id')[:1]
    pacienteDict=None
    if detallesAlta:
        pacienteDict ={
        'entrega_alta':True,
        ...
        'nombrePaciente':paciente.get_name,
        'nombreDoctorAsignado':detallesAlta[0].nombreDoctorAsignado,
        'direccion':paciente.direccion,
        'telefono':paciente.telefono,
        'sintomas':paciente.sintomas,
...
#---------------------------------------------------------------------------------
#------------------------ COMIENZO VISTAS NOSOTROS Y CONTACTANOS ------------------------------
#---------------------------------------------------------------------------------
...
def contactanos_view(request):
    ...
    if request.method == 'POST':
        ...
        if sub.is_valid():
            ...
            send_mail(str(Name)+' || '+str(Email),Mensaje,settings.EMAIL_HOST_USER, settings.EMAIL_RECEIVING_USER, fail_silently = False)
